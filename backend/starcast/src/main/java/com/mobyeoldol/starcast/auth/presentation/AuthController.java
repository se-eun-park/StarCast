...
public class AuthController {
    ...
    public ResponseEntity<BaseResponseTemplate<?>> getAccessCode() {
        ...
        String url = authService.getAccessCode();

        log.info("[login 유효 인가 코드 요청 API] 인가 코드 받아서 프론트로 리턴");
        BaseResponseTemplate<String> successResponse = BaseResponseTemplate.success(url);
    ...
    public ResponseEntity<BaseResponseTemplate<?>> getAccessToken(@RequestParam("code") String code, HttpServletResponse httpServletResponse) {
        ...
        BaseResponseTemplate<Boolean> successResponse;
        ...
        if(optionalAuth.isEmpty()) {
            ...
            successResponse = BaseResponseTemplate.success(true);
        }
        else {
            log.info("[login 카카오에 토큰 요청 API] 클라이언트를 메인 페이지로 리다이렉트");
            successResponse = BaseResponseTemplate.success(false);
        }
